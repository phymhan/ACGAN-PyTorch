...
for epoch in range(opt.niter):
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        aux_errD = aux_criterion(aux_output, label)
        aux_errD.backward()
        optimizerD.step()
        ...
        avg_loss_Aux.update(aux_errD.item(), batch_size)
    ...
    writer.add_scalar('Loss/Aux', avg_loss_Aux.avg, epoch)
    writer.add_scalar('Loss/H_Y', avg_loss_Hy.avg, epoch)
    writer.add_scalar('Metric/MI', avg_loss_Hy.avg - avg_loss_Aux.avg, epoch)
    writer.add_scalar('Metric/Acc', avg_loss_Acc.avg, epoch)
    losses_Aux.append(avg_loss_Aux.avg)
    losses_Acc.append(avg_loss_Acc.avg)
    losses_Hy.append(avg_loss_Hy.avg)

    # do checkpointing
    if epoch % 10 == 0:
        torch.save(netD.state_dict(), '%s/netD_epoch_%d.pth' % (opt.outf, epoch))
    np.save(f'{opt.outf}/losses_Aux.npy', np.array(losses_Aux))
    np.save(f'{opt.outf}/losses_Acc.npy', np.array(losses_Acc))
    np.save(f'{opt.outf}/losses_Hy.npy', np.array(losses_Hy))
