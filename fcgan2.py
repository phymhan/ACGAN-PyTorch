...
for epoch in range(opt.niter):
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        dis_output, _ = netD(fake.detach(), fake_label)
        ...
        dis_output, _ = netD(fake, fake_label)
        errG = dis_criterion(dis_output, dis_label)
        errG.backward()
        optimizerG.step()
        ...
        print('[%d/%d][%d/%d] Loss_D: %.4f (%.4f) Loss_G: %.4f (%.4f) D(x): %.4f D(G(z)): %.4f / %.4f IP: %.4f (%.4f) IQ: %.4f (%.4f) Acc: %.4f (%.4f)'
              % (epoch, opt.niter, i, len(dataloader),
                 ...
                 mi_P.item(), avg_loss_IP.avg, mi_Q.item(), avg_loss_IQ.avg,
